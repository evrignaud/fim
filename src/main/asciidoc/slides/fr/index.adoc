...
* Conserver un journal des modifications pour ces fichiers et afficher par la suite une log détaillées de ces modifications
...
* Elle permet de réflechir et se poser des questions
--
...
* De plus tout les contenus des fichiers modifiés sont conservés ce qui prend un énorme place à la longue
...
Fim ne conserve pas les donnée des fichiers
...
Vous pouvez ajouter un fichier `.fimignore` a tout les niveau du repository,
...
* Utiliser une astérisque pour ignorer plusieurs +
(+++<u>exemple :</u>+++ `*.mp3`)
...
Il n'est pas capable de les éffacer
...
Notamment les backup offline. Pour cela il faut :
...
Fim diagnostique une corruption matérielle si le contenu du fichier à changé alors que les dates de création / modification n'ont pas été modifiées
...
image:important.png[] *Fim peut produire des faux positifs* si le contenu à changé et que les dates ont été réinitialisées
...
* Calcul de la taille et le l'emplacement du prochain block
...
Fim à été mis en OpenSource pour qu'il profite à tous. +
